...
class AsyncOpenaiProcessor:
    ...
    async def process_structured_ouputs_requests(
        ...
        is_deepseek = "deepseek" in model or (request_url and "deepseek" in request_url)
        # Qwen compatibility check
        is_qwen = "qwen" in model or (request_url and "qwen" in request_url) or (request_url and "dashscope" in request_url)
        ...
        if response_format:
            if is_deepseek or is_qwen:
                ...
                system_content_prefix = (
                    "You MUST output a valid JSON object that conforms to the schema below.\n"
                    "Do NOT return the schema definition. Return only the JSON data instance matching the schema.\n"
                    "Make sure the JSON is complete and valid. Do not truncate the output.\n"
                    f"Schema:\n{json.dumps(schema, indent=2)}\n\n"
                )
            else:
                final_response_format = type_to_response_format_param(response_format)
        
        for idx, query in enumerate(queries):
            current_system_content = system_content
            if (is_deepseek or is_qwen) and system_content_prefix:
                current_system_content = system_content_prefix + system_content
            ...
            request = {
                ...
                # Max output tokens, default is often 4096 or lower depending on provider config
                "max_tokens": 8192,
        ...
        with open(save_filepath, "r") as f:
            ...
            for line_number, line in enumerate(f, start=1):
                ...
                # Check if result[1] is a list (indicating errors)
                if isinstance(result[1], list):
                    print(f"[ERROR] Line {line_number}: Request failed with errors: {result[1]}")
                    answer = ""
                elif result[1] is None:
                    print(f"[ERROR] Line {line_number}: Request returned None. This usually means the API call failed silently or returned empty response.")
                    answer = ""
                else:
                    ...
                    try:
                        ...
                        try:
                            ...
                            print(f"DEBUG: LLM Response Content: {answer_content[:500]}...")
                            answer = ""
                    except TypeError as e:
                         print(f"[ERROR] Line {line_number}: Failed to access result fields. Error: {e}. Result: {result[1]}")
                         answer = ""
